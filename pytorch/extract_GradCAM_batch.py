...
class ResNet38_GradCAM(ResNet38):
    ...
    def generate_Gradcam_All(self, output):
        ...
        cams_min = cams.view(batch_size, -1).min(dim=1, keepdim=True)[0].view(batch_size, 1, 1, 1)
        cams_max = cams.view(batch_size, -1).max(dim=1, keepdim=True)[0].view(batch_size, 1, 1, 1)
        cams = (cams - cams_min) / (cams_max - cams_min + 1e-10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cam_save_dir', type=str, default="./AudioCAM")
